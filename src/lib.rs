...
pub struct Serial {
    io: Pin<Box<PollEvented<mio_serial::Serial>>>,
}
...
impl Serial {
    ...
    pub fn pair() -> Result<(Self, Self)> {
        ...
        let master = Serial {
            io: Box::pin(PollEvented::new(master)),
        };
        let slave = Serial {
            io: Box::pin(PollEvented::new(slave)),
        };
        Ok((master, slave))
    }
    ...
    pub fn pair_with_handle(handle: &Handle) -> Result<(Self, Self)> {
        ...
        let master = Serial {
            io: Box::pin(PollEvented::new_with_handle(master, handle)?),
        };
        let slave = Serial {
            io: Box::pin(PollEvented::new_with_handle(slave, handle)?),
        };
        Ok((master, slave))
    }
